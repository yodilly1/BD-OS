...
from app.models.job import JobStatus, create_job, get_job, update_job
    ...
    async def run_deep_search(job_id: str, req: DeepSearchRequest):
        ...
        try:
            results = await prospector.deep_prospecting_flow(
                req.industry, req.size, req.keywords, req.titles, req.limit
            )
...
async def manual_add(request: ManualAddRequest):
    try:
        prospect = await prospector.manual_prospecting_flow(
            request.first_name, request.last_name, request.domain
        )
        if not prospect:
            raise HTTPException(
...
async def search_candidates(
    request: DeepSearchRequest, background_tasks: BackgroundTasks
):
    """
