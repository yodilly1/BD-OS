...
async def main():
    ...
    if candidates:
        print(f"Sample: {candidates[0].get('first_name')} {candidates[0].get('last_name')} - {candidates[0].get('company_name')}")
    
    # Verify NOT in DB (checking by linkedin_url)
    if candidates:
        with Session(engine) as session:
            url = candidates[0]['linkedin_url']
            exists = session.exec(select(Prospect).where(Prospect.linkedin_url == url)).first()
            if not exists:
    ...
    # Use a known company
    url_prospects = await prospector.url_prospecting_flow("https://snorkel.ai", titles=["CEO", "CTO"])
