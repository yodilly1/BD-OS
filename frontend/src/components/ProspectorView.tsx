...
export default function ProspectorView() {
        ...
        try {
          ...
          if (job.status === "completed") {
            ...
            const allUrls = new Set<string>(
              job.result.map((r: any) => r.linkedin_url),
            );
  ...
  const handleSaveCandidates = async () => {
    ...
    setLoading(true);
    try {
      const candidatesToSave = results.filter((r) =>
        selectedCandidates.has(r.linkedin_url),
      );
  ...
  const handleUrlSearch = async () => {
    ...
    try {
      const titlesInput = (
        document.getElementById("url-titles") as HTMLInputElement
      ).value;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 space-y-4">
        ...
        >
          ...
          <span>
            {loading
              ? "Deep Scanning (this takes ~30-60s)..."
              : "Launch Campaign"}
          </span>
        </button>
      </div>
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 space-y-4">
        ...
        <p className="text-sm text-slate-400">
          Know the company? Enter their URL to find employees directly.
        </p>
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 space-y-4">
        ...
        <p className="text-sm text-slate-400">
          Manually add a specific person to enrich.
        </p>
        ...
        <button
          onClick={async () => {
            const first = (
              document.getElementById("manual-first") as HTMLInputElement
            ).value;
            const last = (
              document.getElementById("manual-last") as HTMLInputElement
            ).value;
            const domain = (
              document.getElementById("manual-domain") as HTMLInputElement
            ).value;
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            {savedCount > 0 ? (
              ...
              >
                {loading
                  ? "Saving..."
                  : `Save Selected (${selectedCandidates.size})`}
              </button>
            )}
          </div>
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedCandidates(
                            new Set(results.map((r) => r.linkedin_url)),
                          );
                        } else {
                          setSelectedCandidates(new Set());
                        }
              ...
              <tbody className="divide-y divide-slate-700">
                {results.map((person, idx) => (
                  <tr key={idx} className="hover:bg-slate-700/50">
                    ...
                    <td className="p-3">
                      {person.company_name ||
                        companies.find((c) => c.id === person.company_id)
                          ?.name ||
                        "Unknown"}
                    </td>
