...
export default function SettingsView() {
    ...
    const handleResetDb = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch('http://localhost:8000/api/admin/reset-db', { method: 'POST' });
            if (res.ok) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 space-y-6">
                ...
                <p className="text-slate-400">Application settings and configuration.</p>
            ...
            <div className="bg-red-900/20 p-6 rounded-xl border border-red-700/50 space-y-4">
                ...
                >
                    ...
                    <span>{loading ? 'Resetting...' : 'Reset Database (Delete All Data)'}</span>
                </button>
            </div>
        </div>
    );
