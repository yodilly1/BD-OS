    ...
    const handleGenerate = async () => {
        ...
        try {
            const endpoint = mode === 'email'
                ? `http://localhost:8000/api/outreach/generate-email?prospect_id=${selectedProspectId}&context=${encodeURIComponent(context)}`
                : `http://localhost:8000/api/outreach/generate-linkedin?prospect_id=${selectedProspectId}&context=${encodeURIComponent(context)}`;
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 grid grid-cols-1 gap-6">
                <div className="flex space-x-4">
                    <button
                        ...
                        className={`flex-1 py-2 rounded-lg font-medium ${mode === 'email' ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300'}`}
                    >
                        Email Sequence
                    </button>
                    <button
                        onClick={() => { setMode('linkedin'); setResult(null); }}
                        className={`flex-1 py-2 rounded-lg font-medium ${mode === 'linkedin' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300'}`}
                    >
                        LinkedIn Message
                    </button>
                </div>
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1">Select Prospect</label>
                    <select
                    ...
                    >
                        ...
                        {prospects.map(p => (
                            <option key={p.id} value={p.id}>
                                {p.first_name} {p.last_name} - {p.title} ({companies.find(c => c.id === p.company_id)?.name})
                            </option>
                        ))}
                    </select>
                </div>
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1">Context / Goal</label>
                    <textarea
                        value={context}
