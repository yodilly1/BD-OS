...
async def test_deep_prospecting():
    ...
    try:
        ...
        prospects = await prospector.deep_prospecting_flow(industry, size, keywords, titles)
        
        print(f"   Result: Found {len(prospects)} prospects.")
        
        if len(prospects) == 0:
            print("   WARNING: No prospects found. This might be due to API limits or mock data.")
        else:
            for p in prospects:
            ...
            interaction = await outreach.generate_email_sequence(target_prospect.id, context)
            ...
            if "Vayu" in interaction.content or "usage-based" in interaction.content.lower():
