...
async def verify_full_flow():
    ...
    prospects = await prospector.url_prospecting_flow(
        target_company, titles=[target_role]
    )
    ...
    print(
        f"   Target: {target_prospect.first_name} {target_prospect.last_name} ({target_prospect.title})"
    )
